...
Emenda aprovada na retomada de 22/09/2026: antes do consentimento, `hello` transmite apenas `connection_id`, `tab_id`, `meeting_hint` (código de sala normalizado), `active` e relógios do cliente. O bridge guarda esses sinais somente em memória. O app seleciona o `meeting_hint` apenas quando há exatamente um código ativo; se houver zero ou mais de um, a sessão não recebe nomes da extensão até existir vínculo inequívoco. A `meeting_key` continua opaca e é vinculada no servidor ao código selecionado, sem derivá-la do nome/título. `hello` pode atualizar `active` antes do consentimento; legenda/nome não é persistido nesse período. O código bruto não entra em job, log nem artefato.
