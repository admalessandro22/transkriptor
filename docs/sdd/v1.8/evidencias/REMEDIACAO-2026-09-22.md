...
- O primeiro CI (`36001436434`) falhou na suíte Python: **802 passed, 2 failed**. Os dois testes de reprodução COM em `tests/test_com_audio.py` exigiam um endpoint físico de loopback e terminaram com `SEM RESULTADO` no runner; a instalação limpa, auditoria e SBOM haviam passado. O teste foi corrigido com uma sonda independente de dispositivo: no runner sem endpoint, esses dois casos recebem `skip` explícito; em máquina local com endpoint, continuam executando a reprodução real e falham se ela regredir. A falha do primeiro CI não é contabilizada como gate verde; novo SHA e workflow são exigidos.
- RED dirigido: `pytest tests/test_com_audio.py::test_runner_sem_loopback_registra_skip_em_vez_de_falha_de_com -q --tb=short` exit 1 com `SEM RESULTADO` tratado como falha de COM. GREEN após a sonda: `pytest tests/test_com_audio.py -q --tb=short` exit 0, **8 passed** em 3,60 s; suíte local completa exit 0, **805 passed** em 124,63 s. A sonda é independente da reprodução destrutiva e só permite `skip` no ambiente `GITHUB_ACTIONS` quando loopback não está disponível; em máquina local, falta do endpoint falha explicitamente.
