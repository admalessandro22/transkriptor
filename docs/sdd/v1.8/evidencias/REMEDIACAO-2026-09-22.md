...
## Uso operacional autorizado em 24/09/2026

- O usuário dispensou a execução dos testes reais e autorizou atualizar a bandeja ativa e publicar o trabalho. Essa dispensa registra ausência de evidência, **não** resultado PASS nem aceite formal G3. Permanecem sem execução: captura 25/600 s com Whisper real, D2 em Chrome e Edge instalados, reunião consentida de três pessoas, instalação CUDA limpa e segunda partida real da bandeja. A política de sugestões/abstenção de nomes continua ativa.
- No SHA `5566f074011948457571cf934c8fb02c8855a6e8`, após o commit documental: `pytest tests/ -q --tb=short` exit 0, **804 passed** em 98,64 s; `verificar_fase.py --fase all` exit 0, rastreabilidade OK e **238 passed** em 26,84 s; `compileall`, `npm ci` (84 pacotes, zero alertas npm), Vitest **22 passed**, Playwright **21 passed**, `pip check` da venv CPU e `git diff --check` exit 0. Instalação CPU isolada da Task 11 continua válida no código; a documentação do commit não altera o instalador.
- Auditoria local da venv CPU em 24/09: `pip_audit --no-deps --disable-pip -r requirements/requirements-cpu.lock --ignore-vuln PYSEC-2026-3447 --ignore-vuln PYSEC-2025-194` exit 0, nenhuma vulnerabilidade adicional conhecida, **3 entradas ignoradas** para as duas exceções temporárias de `docs/DEPENDENCIAS.md`; `gerar_sbom.py` produziu JSON idêntico ao SBOM versionado; pré-checagem CPU exit 0. As exceções continuam sem aceite explícito para release formal.
- A instância antiga `pythonw.exe` PID 16412 apontava para este checkout. Antes do reinício autorizado: **0** arquivos de áudio modificados nos 30 minutos anteriores, **0** workers, fila com 51 `ready`, 2 `failed`, **0** `pending`/`processing`. O monitor antigo havia emitido erro crítico; apenas esse PID foi encerrado. A nova bandeja pelo mesmo Python CUDA iniciou no PID 22196, registrou `Bandeja pronta`, e a inspeção encontrou exatamente uma instância. Não foi capturado áudio nem lido conteúdo de transcrição para essa checagem. O Python global passou na pré-checagem CUDA, mas `pip check` global relata conflitos preexistentes de `pdfplumber`/`pdfminer-six`/Pillow; a instalação CUDA limpa continua sem validação.
- A branch `remediacao-auditoria-v18` foi publicada em `origin` no SHA `5566f074011948457571cf934c8fb02c8855a6e8`; workflow Windows CPU `36001436434` foi acionado. Registrar conclusão e SHA final abaixo quando o CI terminar. `config.VERSAO` permanece `1.7.0`: não houve bump, tag, merge ou release formal com gates aprovados.
- O primeiro CI (`36001436434`) falhou na suíte Python: **802 passed, 2 failed**. Os dois testes de reprodução COM em `tests/test_com_audio.py` terminaram com `SEM RESULTADO`; instalação limpa, auditoria e SBOM haviam passado. Esse resultado **não prova** ausência de dispositivo: `derrubar_com()` rodava fora do `try`, e qualquer exceção ali também produzia `SEM RESULTADO`. A reprodução depende de alto-falante/loopback, que o runner pode não possuir. A falha do primeiro CI não é contabilizada como gate verde.
- RED dirigido inicial: `pytest tests/test_com_audio.py::test_runner_sem_loopback_registra_skip_em_vez_de_falha_de_com -q --tb=short` exit 1 com `SEM RESULTADO` tratado como falha de COM. Uma primeira sonda ampla gerou 8/8 no teste dirigido e 805/805 na suíte local, mas revisão independente mostrou que ela podia ocultar falhas de COM como indisponibilidade de dispositivo. Por isso, o SHA `903dcd3e588090a779ea18646cf91a2c69892cb2` e seu CI não são aceitos como verificação final, mesmo se o workflow terminar verde.
- A sonda revisada só classifica `ABSENT` quando `soundcard.all_speakers()` retorna lista vazia em subprocesso; exceção, timeout e saída anormal são `ERROR` e **falham**. `skip` ocorre somente em `GITHUB_ACTIONS` com `ABSENT`. A reprodução COM real continua obrigatória se existir alto-falante. O `try` agora cobre `derrubar_com()`, preservando a exceção diagnóstica em vez de `SEM RESULTADO`. RED do teste de distinção: exit 1; GREEN dirigido: `pytest tests/test_com_audio.py -q --tb=short` exit 0, **9 passed** em 2,05 s; suíte local completa **806 passed** em 104,58 s. CI do novo SHA ainda deve ser executado.
- O segundo CI (`36002565120`) terminou verde no SHA `903dcd3e588090a779ea18646cf91a2c69892cb2`: Python **803 passed, 2 skipped**, JS e E2E verdes, audit/SBOM verdes. Esse SHA foi rejeitado pela revisão porque o `skip` ainda poderia esconder erro de COM; não é o candidato final. O novo commit com enumeração explícita e seu CI substituem esse resultado.
